...
def test_shooting_weight_with_excluded_tail_can_boost_the_last_three_nodes() -> None:
    """The torque regularization selector should support stronger terminal penalties."""

    weights = _shooting_weight_with_excluded_tail(10, 3, 3.0)

    assert np.allclose(weights[:7], 1.0)
    assert np.allclose(weights[7:], 3.0)
